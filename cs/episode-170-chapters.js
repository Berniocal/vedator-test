(()=>{
  if(window.__vedatorEpisode170Chapters)return;window.__vedatorEpisode170Chapters=true;
  const CHAPTERS=[65,126,334,377,489,693,780,909,1055,1118,1187,1345,1455,1545,1616];
  ...
  function install(){
    ...
    if(!audio||!title||!row||row.dataset.episode170Bound)return false;
    row.dataset.episode170Bound='1';
    const sync=()=>{if(episodeNumber(title.textContent)!==170)return;const t=audio.currentTime||0;const previous=row.querySelector('.previous-question'),next=row.querySelector('.next-question');if(previous)previous.disabled=!CHAPTERS.some(x=>x<t-1);if(next)next.disabled=!CHAPTERS.some(x=>x>t+1)};
    row.addEventListener('click',event=>{if(episodeNumber(title.textContent)!==170)return;const button=event.target.closest('.vedator-question-btn');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const t=audio.currentTime||0;const target=button.classList.contains('previous-question')?[...CHAPTERS].reverse().find(x=>x<t-1):CHAPTERS.find(x=>x>t+1);if(Number.isFinite(target)){audio.currentTime=target;audio.play().catch(()=>{});sync()}},true);
