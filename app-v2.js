(()=>{
  ...
  function saveProgress(force=false,ended=false){
    ...
    writeJson(PROGRESS_KEY,state.progress);saveCollectionProgress(time,duration,completed);refreshEpisodeCard(current.episode.number);if(force||completed!==Boolean(previous.completed))refreshSeriesProgress();
  }
  ...
  async function start(){
    ...
    try{
      ...
      buildLegacyQuestionIndex();loadUserData();installEpisodeExperienceStyles();installUiExperience();installFullParityUi();applyStaticUi();renderEpisodes();renderSeries();renderQuestions();renderNonQuestions();renderPlaylists();renderData();bind();setView('episodes');
