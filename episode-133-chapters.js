(()=>{
  if(window.__vedatorEpisode133Chapters)return;window.__vedatorEpisode133Chapters=true;
  const CHAPTERS=[60,232,513,682,811,975,1026,1091,1184,1317];
  ...
  function install(){
    ...
    if(!audio||!title||!row||row.dataset.episode133Bound)return false;
    row.dataset.episode133Bound='1';
    const sync=()=>{if(episodeNumber(title.textContent)!==133)return;const t=audio.currentTime||0;const previous=row.querySelector('.previous-question'),next=row.querySelector('.next-question');if(previous)previous.disabled=!CHAPTERS.some(x=>x<t-1);if(next)next.disabled=!CHAPTERS.some(x=>x>t+1)};
    row.addEventListener('click',event=>{if(episodeNumber(title.textContent)!==133)return;const button=event.target.closest('.vedator-question-btn');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const t=audio.currentTime||0;const target=button.classList.contains('previous-question')?[...CHAPTERS].reverse().find(x=>x<t-1):CHAPTERS.find(x=>x>t+1);if(Number.isFinite(target)){audio.currentTime=target;audio.play().catch(()=>{});sync()}},true);
