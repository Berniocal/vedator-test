(()=>{
  if(window.__vedatorEpisode343SummaryInteractive)return;
  window.__vedatorEpisode343SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode343-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode343Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode343Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:343,time:seconds,createdAt:Date.now()};
    play.click();
  }
