(()=>{
  if(window.__vedatorEpisode347SummaryInteractive)return;
  window.__vedatorEpisode347SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode347-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode347Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode347Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:347,time:seconds,createdAt:Date.now()};
    play.click();
  }
