(()=>{
  if(window.__vedatorEpisode345SummaryInteractive)return;
  window.__vedatorEpisode345SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode345-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode345Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode345Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:345,time:seconds,createdAt:Date.now()};
    play.click();
  }
