(()=>{
  if(window.__vedatorEpisode338SummaryInteractive)return;
  window.__vedatorEpisode338SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode338-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode338Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode338Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:338,time:seconds,createdAt:Date.now()};
    play.click();
  }
