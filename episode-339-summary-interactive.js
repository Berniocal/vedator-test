(()=>{
  if(window.__vedatorEpisode339SummaryInteractive)return;
  window.__vedatorEpisode339SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode339-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode339Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode339Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:339,time:seconds,createdAt:Date.now()};
    play.click();
  }
