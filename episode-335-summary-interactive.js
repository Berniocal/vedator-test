(()=>{
  if(window.__vedatorEpisode335SummaryInteractive)return;
  window.__vedatorEpisode335SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode335-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode335Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode335Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:335,time:seconds,createdAt:Date.now()};
    play.click();
  }
