(()=>{
  if(window.__vedatorEpisode334SummaryInteractive)return;
  window.__vedatorEpisode334SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode334-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode334Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode334Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:334,time:seconds,createdAt:Date.now()};
    play.click();
  }
