(()=>{
  if(window.__vedatorEpisode119Chapters)return;window.__vedatorEpisode119Chapters=true;
  const CHAPTERS=[97,183,286,410,549,635,761,937,1033,1163,1319,1431,1547,1588];
  ...
  function install(){
    ...
    if(!audio||!title||!row||row.dataset.episode119Bound)return false;
    row.dataset.episode119Bound='1';
    const sync=()=>{if(episodeNumber(title.textContent)!==119)return;const t=audio.currentTime||0;const previous=row.querySelector('.previous-question'),next=row.querySelector('.next-question');if(previous)previous.disabled=!CHAPTERS.some(x=>x<t-1);if(next)next.disabled=!CHAPTERS.some(x=>x>t+1)};
    row.addEventListener('click',event=>{if(episodeNumber(title.textContent)!==119)return;const button=event.target.closest('.vedator-question-btn');if(!button)return;event.preventDefault();event.stopImmediatePropagation();const t=audio.currentTime||0;const target=button.classList.contains('previous-question')?[...CHAPTERS].reverse().find(x=>x<t-1):CHAPTERS.find(x=>x>t+1);if(Number.isFinite(target)){audio.currentTime=target;audio.play().catch(()=>{});sync()}},true);
