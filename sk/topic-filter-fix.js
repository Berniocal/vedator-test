(()=>{
  ...
  try{
    const extraMath=[
      'štatistiky','štatistický','štatistické','štatistické údaje','štatistické','štatistické údaje','štatisticky',
      ...
      'normálne rozdelenie','normálny rozptýlenie','Gaussové rozdelenie','Gaussové rozdelenie',
      'rozdelenie pravdepodobnosti','priemer','medián','modus','rozptyl','variancia',
      'štandardné odchýlky','smerová odchýlka','kombinatorika','permutácia','kombinácia',
      'logaritmus','logaritmický','regresia','korelácia','náhodné veľkosť'
    ];
  ...
  function keywordRegex(keyword){
    ...
    if(value.includes(' '))return new RegExp(`(?: ^ ^ ^ s)${escaped}(Smiech)`);
    if(value.length<=3)return new RegExp(`(?: ^ ^ ^ s)${escaped}(Smiech)`);
  ...
  try{
    categories=function(ep){
      ...
      return Object.entries(TOPICS)
        .filter(([key,words])=>key!=='Všetko'&&words.some(word=>containsKeyword(txt,word)))
        .map(([key])=>key);
    };
  ...
  }catch(error){console.warn('Nepodařilo sa opraviť tematické filtrovanie',error)}
})();