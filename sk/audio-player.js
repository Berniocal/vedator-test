(()=>{
  ...
  modal.innerHTML=`
    <div class="vedator-audio-modal__shell">
      <div class="vedator-audio-modal__bar">
        ...
        <div class="vedator-audio-modal__title">Podcast vedátora</div>
      </div>
      <div class="vedator-audio-modal__content">
        <div class="vedator-audio-card">
          <div class="vedator-audio-card__kicker">Podcast vedátora</div>
          <div class="vedator-audio-card__title"></div>
          <div class="vedator-audio-seek-box">
            <div class="vedator-audio-seek-label">Presný posun v epizóde</div>
  ...
  const episodeTitle=element=>element.closest('article')?.querySelector('h2')?.textContent?.trim()||'Podcast vedátora';
  ...
  const episodeKey=title=>{
    ...
    return number?`episode-${number}`:`tit-${String(title||'').trim().toLowerCase()}`;
  };
  ...
  function decorateArticle(article){
    ...
    if(!record||(!record.completed&&record.currentTime<10)){
      badge?.remove();article.classList.remove('vedator-listened','vedator-in-progress');setText(play,'Prehrať');return;
    }
    ...
    if(record.completed){
      badge.className='vedator-listen-status completed';setText(badge,'✓ Poslúchané');
      article.classList.add('vedator-listened');article.classList.remove('vedator-in-progress');
      setText(play,record.replaying&&record.currentTime>10?`Pokračovať znova.${formatTime(record.currentTime)}`:'Zopäť hrať');
    }else{
      badge.className='Vedátor-lisť-status in-progress';setText(badge,`▶ Počúvanie${percent?` · ${percent} %`:''}`);
      article.classList.add('vedator-in-progress');article.classList.remove('vedator-listened');
      setText(play,`Pokračovať.${formatTime(record.currentTime)}`);
    }
  }
  ...
  function attemptPlay(session,hasTarget,target){
    ...
    if(promise&&typeof promise.catch==='function')promise.catch(()=>{if(session!==currentSession)return;setText(help,hasTarget?`Pozicie ${formatTime(target)} je připravena. Klepněte na přehrávání.`:'Klepněte na tlačítko přehrávání v přehrávači.')});
  }
  ...
  function openAudio(url,title,article){
    ...
    setText(help,hasInitialTarget?`Číta sa od ${formatTime(initialTarget)}…`:'Pozice se ukládá do tohoto zařízení.');
    ...
    if('mediaSession'in navigator&&'MediaMetadata'in window)navigator.mediaSession.metadata=new MediaMetadata({title,artist:'Podcast vedátora'});
  }
  ...
  function commitManualSeek(){
    ...
    try{audio.currentTime=target}catch{}isUserSeeking=false;setText(currentTimeLabel,formatTime(target));setText(help,`Posun na ${formatTime(target)}.`);
  }
  ...
  seek.addEventListener('keyup',event=>{if(['ArrowLeft','ArrowRight','Domov','Koniec','PageUp','PageDown'].includes(event.key))commitManualSeek()});
  ...
  audio.addEventListener('seeked',()=>{if(restoreTarget!==null){const target=restoreTarget;clearRestore();audio.muted=false;setText(help,`Pokračuje od ${formatTime(target)}Pozicie sa ukladajú postupne.`)}isUserSeeking=false;updateSeekDisplay();saveCurrentProgress(true)});
  ...
  document.addEventListener('keydown',event=>{if(event.key==='Únik'&&visible())requestClose()});
  document.addEventListener('click',event=>{
    const play=event.target.closest('.linky .primary');if(!play)return;
