(()=>{
  if(window.__vedatorEpisode342SummaryInteractive)return;
  window.__vedatorEpisode342SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode342-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode342Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode342Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:342,time:seconds,createdAt:Date.now()};
    play.click();
  }
