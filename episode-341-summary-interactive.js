(()=>{
  if(window.__vedatorEpisode341SummaryInteractive)return;
  window.__vedatorEpisode341SummaryInteractive=true;

  const selector='.episode-summary[data-vedator-episode341-summary="1"]';
  ...
  function enhanceTime(node){
    ...
    node.dataset.vedatorEpisode341Seconds=String(seconds);
  ...
  function playFrom(node,event){
    ...
    const seconds=Number(node.dataset.vedatorEpisode341Seconds);
    if(!play||!Number.isFinite(seconds))return;

    event?.preventDefault();
    event?.stopPropagation();
    window.__vedatorQuestionContext=null;
    window.__vedatorRequestedStart={episode:341,time:seconds,createdAt:Date.now()};
    play.click();
  }
